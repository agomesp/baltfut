...
export function CommunityBar({
  ...
  const cell = (pct: number, label: string, color: string) => (
    <span style={{ color }}>
      <b style={{ fontFamily: SAIRA, fontWeight: 800, fontSize: 20 }}>{pct}%</b>{" "}
      <span style={{ fontFamily: JB, fontSize: 8.5, color: "#9bb6a6" }}>{label}</span>
    </span>
  );
  return (
    <div style={{ borderRadius: 11, padding: "10px 13px", background: "linear-gradient(120deg, rgba(200,255,45,0.07), rgba(200,255,45,0) 70%)", border: "1px solid rgba(200,255,45,0.16)" }}>
      {withMarker ? <div style={{ marginBottom: 7 }}><SectionLabel style={{ letterSpacing: "0.14em" }}>{"// A COMUNIDADE PALPITA"}</SectionLabel></div> : null}
      {total === 0 ? (
